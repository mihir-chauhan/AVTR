    ...
    const handleAudioAccess = async () => {
        ...
        try {
            ...
            if (SpeechRecognition) {
                ...
                recognition.onresult = (event) => {
                    ...
                    if (currentText) {
                        ...
                        if (finalTranscript) {
                            ...
                            if (finalTranscript.toLowerCase().includes("richard")) {
                                stopEverything();
                                navigate('/call/user/avatar-room');
                            } else {
                                setError("You are not connected with user");
                                setTimeout(() => {
                                    setError(null);
                                    setTranscript('');
                                    transcriptRef.current = '';
                            ...
                            silenceTimer.current = setTimeout(() => {
                                ...
                                if (interimTranscript.toLowerCase().includes("richard")) {
                                    stopEverything();
                                    navigate('/call/user/avatar-room');
                                } else {
                                    setError("You are not connected with user");
                ...
                // Failsafe: if recognition stops for any reason
                recognition.onend = () => {
                    ...
                    if (current) {
                        ...
                        if (current.toLowerCase().includes("richard")) {
                            stopEverything();
                            navigate('/call/user/avatar-room');
                        } else {
                            setError("You are not connected with user");
